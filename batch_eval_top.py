...
def create_patch_batch_list(filename,
    ...
    if num_channels == 3:
        ...
        ndsm_img = imread(join(ndsm_dir, filename.replace('input_img', 'dsm').replace('_mosaic', '').replace('area', 'matching_area') + '_normalized.jpg'), -1)
        ndsm_img = ndsm_img[:, :, np.newaxis]
        dsm_dir = join(data_dir, 'dsm')
        dsm_img = imread(join(dsm_dir, filename.replace('input_img', 'dsm').replace('_mosaic', '').replace('area', 'matching_area') + '.png'), -1)
