    ...
    for directory in directories:
        ...
        if not file_list:
            ...
            for f in file_list:
                ...
                annotation_file = join(image_dir, "ground_truth", filename + ".png")
...
def read_dataset_potsdam_submission(image_dir):
    pickle_filename = "Potsdam_submission.pickle"
    pickle_filepath = join(image_dir, pickle_filename)
    if not exists(pickle_filepath):
        result = create_image_list_potsdam_submission(image_dir)
        print("pickling...")
        with open(pickle_filepath, "wb") as f:
            pickle.dump(result,f,pickle.HIGHEST_PROTOCOL)
    else:
    ...
    with open(pickle_filepath,"rb") as f:
        result = pickle.load(f)
        training_records = result['training_set_submission']
        validation_records = result['validation_set_submission']
        del result
    return training_records, validation_records


def create_image_list_potsdam_submission(image_dir):
    if not gfile.Exists(image_dir):
        print("Image directory '" + image_dir + "' not found.")
        return None
    directories = ['training_set_submission', 'validation_set_submission']
    image_list = {}
    for directory in directories:
        file_list = []
        image_list[directory] = []
        ...
        if not file_list:
            ...
            for f in file_list:
                filename = os.path.splitext(f.split("/")[-1])[0]
                annotation_file = join(image_dir, "ground_truths_submission", filename + ".png")
                if exists(annotation_file):
                    record = {'image': f, 'annotation': annotation_file, 'filename': filename}
                    image_list[directory].append(record)
                else:
...
def create_image_list_15_test(image_dir):
    if not gfile.Exists(image_dir):
        print("Image directory '" + image_dir + "' not found.")
        return None
    directories = ['training_set_15c_test', 'validation_set_15c_test']
    image_list = {}
    for directory in directories:
        file_list = []
        image_list[directory] = []
        ...
        if not file_list:
            ...
            for f in file_list:
                filename = os.path.splitext(f.split("/")[-1])[0]
                annotation_file = join(image_dir, "ground_truths_15c_test", filename + ".png")
                if exists(annotation_file):
                    record = {'image': f, 'annotation': annotation_file, 'filename': filename}
                    image_list[directory].append(record)
                else:
...
def read_dataset_15_test(image_dir):
    pickle_filename = "Vaihingen_15_test.pickle"
    pickle_filepath = join(image_dir, pickle_filename)
    if not exists(pickle_filepath):
        result = create_image_list_15_test(image_dir)
        print("pickling...")
        with open(pickle_filepath, "wb") as f:
            pickle.dump(result,f,pickle.HIGHEST_PROTOCOL)
    else:
    ...
    with open(pickle_filepath,"rb") as f:
        result = pickle.load(f)
        training_records = result['training_set_15c_test']
        validation_records = result['validation_set_15c_test']
        del result
    return training_records, validation_records
...
def create_image_list_15(image_dir):
    if not gfile.Exists(image_dir):
        print("Image directory '" + image_dir + "' not found.")
        return None
    directories = ['training_set_15c', 'validation_set_15c']
    image_list = {}
    for directory in directories:
        file_list = []
        image_list[directory] = []
        ...
        if not file_list:
            ...
            for f in file_list:
                filename = os.path.splitext(f.split("/")[-1])[0]
                annotation_file = join(image_dir, "ground_truths_15c", filename + ".png")
                if exists(annotation_file):
                    record = {'image': f, 'annotation': annotation_file, 'filename': filename}
                    image_list[directory].append(record)
                else:
...
def read_dataset_15(image_dir):
    pickle_filename = "Vaihingen_15.pickle"
    pickle_filepath = join(image_dir, pickle_filename)
    if not exists(pickle_filepath):
        result = create_image_list_15(image_dir)
        print("pickling...")
        with open(pickle_filepath, "wb") as f:
            pickle.dump(result,f,pickle.HIGHEST_PROTOCOL)
    else:
    ...
    with open(pickle_filepath,"rb") as f:
        result = pickle.load(f)
        training_records = result['training_set_15c']
        validation_records = result['validation_set_15c']
        del result
    return training_records, validation_records
